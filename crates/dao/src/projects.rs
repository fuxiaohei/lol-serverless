use crate::{
    deploys::{DeployType, Status as DeployStatus},
    models::{deploys, playground, project},
    users, DB,
};
...
async fn create_internal(
    ...
    let mut project = project::Model {
        ...
        deploy_status: DeployStatus::WaitDeploy.to_string(),
...
pub async fn set_deploy_status(id: i32, status: DeployStatus, msg: &str) -> Result<()> {
    let db = DB.get().unwrap();
    let now = chrono::Utc::now();
    project::Entity::update_many()
        .col_expr(
...
pub async fn create_deploy(project: &project::Model, dtype: DeployType) -> Result<deploys::Model> {
...
pub async fn update_source(id: i32, source: String) -> Result<deploys::Model> {
    ...
    // create new deploy
    create_deploy(&project, DeployType::Production).await
}
