...
pub async fn init_envs(dir: String) -> Result<()> {
    ...
    for entry in walkdir::WalkDir::new(dir) {
        ...
        let envs = match land_helpers::crypt::decode(&env_content.secret, &env_content.values) {
            Ok(v) => v,
            Err(e) => {
...
pub async fn read_file(file: &str) -> Result<()> {
    ...
    let envs = land_helpers::crypt::decode(&env_content.secret, &env_content.values)?;
    let mut global_envs = ENVS.lock().await;
    global_envs.insert(basepath.to_string(), envs);
