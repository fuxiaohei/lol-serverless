use crate::{
    ...
    ENABLE_WASMTIME_AOT, ENDPOINT_NAME,
};
use anyhow::Result;
use axum::{
    body::{Body, HttpBody},
...
use land_wasm_host::{Context, Worker};
...
pub async fn run(
    ...
    // call wasm async
    async move {
        let result = call_wasm(req, &info).await;
...
async fn call_wasm(req: Request<Body>, info: &WorkerInfo) -> Result<Response<Body>> {
    ...
    let wasm_req = land_wasm_host::HostCallRequest {
