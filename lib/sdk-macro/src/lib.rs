...
pub fn http_main(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let iface_impl1 = quote!(
        ...
        impl context::Guest for WorkerHttpImpl {
            fn is_pending() -> bool{
                let ctx = ExecutionCtx::get_ctx();
                ctx.is_pending()
            }

            fn select() -> bool {
                let mut ctx = ExecutionCtx::get_ctx();
                ctx.execute();
                ctx.is_pending()
            }
    ...
    let iface_impl2 = if func_args_len == 2 {
        iface_impl2_1
    } else {
        iface_impl2_2
    };
